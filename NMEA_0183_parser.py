...
class NMEA_parser:
  ...
  def __init__(self):
      ...
      self.__sensor_values={  "MTW":["temprature","Unit"],
                              "DPT":["relative_to_transuducer","Offsett"],
                              "DBT":["depth_below_transducer","Unit","depth_below_transducer","Unit","depth_below_transducer","Unit"],
                              "GGA":["UTC time","1-sigma error, latitude","1-sigma error, longitude","1-sigma error, altitude","ID of most likely failed satellite","Probability of missed detection","Estimate of bias, meters","Standard deviation of bias estimate"],
                              "GLL":["Latitude","N or S ","Longitude","E or W","UTC of this position","Status A - Data Valid, V - Data Invalid"],
                              "GSV":[ "number_of_messages","message_number","satellites_in_view","satellite_number","elevation_degrees","azimuth_degrees_true","SNR_dB_(00-99)","satellite_number","elevation_degrees","azimuth_degrees_true","SNR_dB_(00-99)","satellite_number","elevation_degrees","azimuth_degrees_true","SNR_dB_(00-99)","satellite_number","elevation_degrees","azimuth_degrees_true","SNR_dB_(00-99)","satellite_number","elevation_degrees","azimuth_degrees_true","SNR_dB_(00-99)"],
                              "HDM":["Heading Degrees","Unit"],
                              "HDT":["Heading_degrees_True","T=true"],
                              "MWV":["Wind_Angle","R=Relative,T=True","Wind_Speed","Units","A=Valid_V=Invalid"],
                              "RMC":["time","status_V=warning","latitude","north_south","longitude","east_west","speed_in_knots","track_made_good_true_deg","date_ddmmyy","magnetic_variation_deg","east_west"],
                              ...
                              "VHW":["degrees_true","T=true","degrees_mag","Unit","speed","Unit","speed","Unit"],
                              "VTG":["track_degrees_true","T=true","Track_degrees","Unit","speed","unti","speed","Unit"],
                              "VWR":["wind_direction_degrees","relative_wind_dirrection","speed","Unit","speed","Unit","speed","Unit"],
                              ...
                              "AAM":["Status_A=Arrival_circle_entered","Status_A=perpendicular_passed_at_waypoint","Arrival_circle_radius","Unit","Waypoint_ID"],
                              ...
                              "APB":["Status_V=LORAN-C_Blink_or_SNR_warning_V=general_warning_flag_or_other_navigation_systems_when_a_reliable_fix_is_not_available","Status_V=Loran-C_Cycle_Lock_warning_flag_A=OK_or_not_used","Cross_Track_Error_Magnitude","Direction_to_steer_L_or_R","Unit","Status_A=Arrival_Circle_Entered","Status_A=Perpendicular_passed_at_waypoint","Bearing_origin_to_destination","Unit","Destination_Waypoint_ID","Bearing_present_position_to_Destination","Unit","Heading_to_steer_to_destination_waypoint","Unit"], 
                              ...
                              "VDM":["Time_(UTC)","MMSI_Number","Latitude","Longitude","Speed_in_Knots","Heading","Course_over_Ground","Rate_of_turn","Navigation_status"], 
                              ...
                              "OSD":["Heading_degrees_true","Status_A=Data_Valid","Vessel_Course_degrees_True","Course_Reference","Vessel_Speed","Speed_Reference","Vessel_Set_degrees_True","Vessel_drift(speed)","Units"], 
  ...
  def __order_data(self, data,data_id):
       ...
       if "Unit" in self.__sensor_values[data_id]:
           indexis= [i for i, s in enumerate(self.__sensor_values[data_id]) if s == "Unit" or s == "Unit"]
           
 
               
           for i in indexis[::-1]:
               
               s = "%s_in_%s" % (self.__sensor_values[data_id][i-1],data.pop(i))
